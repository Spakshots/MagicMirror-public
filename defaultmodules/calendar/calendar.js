...
Module.register("calendar", {
	...
	defaults: {
		...
		calendars: [
			{
				symbol: "calendar-alt",
				url: "https://calendar.google.com/calendar/embed?src=sjb2019dpo%40gmail.com&ctz=America%2FLos_Angeles"
			}
		],
		customEvents: [
	...
	socketNotificationReceived (notification, payload) {
		...
		if (notification === "CALENDAR_EVENTS") {
			...
			if (this.calendarData[payload.url].checksum === payload.checksum) {
				// then don't update the pm2 start npm --name 'magicmirror' -- startUI
				return;
			}
